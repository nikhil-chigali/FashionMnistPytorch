...
def get_loader(is_train, batch_size):
    ...
    if is_train:
        ...
        train_loader = DataLoader(
            train_dataset, batch_size=batch_size, num_workers=4, shuffle=is_train
        )
        val_loader = DataLoader(
            val_dataset, batch_size=batch_size, num_workers=4, shuffle=is_train
        )
        loader = (train_loader, val_loader)
    else:
        loader = DataLoader(
            dataset, batch_size=batch_size, num_workers=4, shuffle=is_train
        )
    return loader
